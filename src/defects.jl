...
function mkGrapheneSystem(
    ...
    # Assemble Δ
    Δ = [get!(pert_dict, (x, y), 0.0 + 0.0im)*I(2) for x in all_atoms, y in all_atoms]
    ...
    # Assemble V
    V_array = [
            [(sum([((atom == c[2]) * c[1]) for c in imp.coupling])::Float64)*I(2) for atom in all_atoms]
            for imp in imps
    ]

    if !isempty(V_array)
        V_init = reduce(hcat, V_array)'
        V = hvcat(size(V_init,1),(V_init...))
    else
